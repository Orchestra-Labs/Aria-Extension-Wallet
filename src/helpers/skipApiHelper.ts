...
// const executeSkipExchange = async ({
//   step,
//   skipRoute,
//   operationIndex,
//   sendObject,
...
//   try {
//     // Get addresses for all chains in the route
//     const userAddresses: UserAddress[] = await Promise.all(
//       skipRoute.requiredChainAddresses.map(async (chainId: string) => {
...
//       // For simulation, run fee calculations
//       const simulationResult = await runSkipFeeSimulations(skipRoute, userAddresses);
//       const calculatedFees = calculateSkipFees(skipRoute, simulationResult);
...
//           route: skipRoute,
//           estimatedAmountOut: skipRoute.estimatedAmountOut,
//           fees: feeStructure,
//         },
//       };
//     } else {
...
//         skipRoute,
//         userAddresses,
//         async (chainId: string) => await getCosmosSigner(chainId),
//         getEvmSigner,
//         getSvmSigner,
//       );
...
//     }
//   } catch (error) {
//     console.error('[executeSkipExchange] Error:', error);
//     return {
//       success: false,
//       message: error instanceof Error ? error.message : 'Skip exchange failed',
//     };
//   }
// };

// const updateOutputWithFees = async (
//   step: TransactionStep,
...
//   let outputAmount = stepLog.outputAmount;
//   let inputAmount = stepLog.inputAmount;

//   // For Skip operations, use the route information to calculate actual outputs
//   if (skipRoute && operationIndex !== undefined) {
//     const operation = skipRoute.operations[operationIndex];
//     if (operation) {
//       // Subtract fees from the output amount
//       const operationFees = calculateOperationFees(operation, result);
//       const netOutput = BigInt(operation.amountOut || '0') - BigInt(operationFees.totalFees);

//       outputAmount = netOutput.toString();

//       // Update the log with the net output
//       set(updateStepLogAtom, {
//         stepHash: step.hash,
//         log: {
//           outputAmount,
//           inputAmount: operation.amountIn || inputAmount,
//         },
//       });

//       // If this is part of a multi-step route, update the next step's input
//       if (operationIndex < skipRoute.operations.length - 1) {
//         const nextOperationIndex = operationIndex + 1;
//         const nextOperation = skipRoute.operations[nextOperationIndex];
//         const nextStepHash = findStepHashForOperation(nextOperationIndex);

//         if (nextStepHash && nextOperation) {
//           // Set the next operation's input to this operation's net output
//           set(updateStepLogAtom, {
//             stepHash: nextStepHash,
//             log: {
//               inputAmount: netOutput.toString(),
//               operationIndex: nextOperationIndex,
//               skipRoute, // Pass the route to the next step
//             },
//           });
//         }
//       }
...
// const calculateOperationFees = (operation: any, result: TransactionResult) => {
//   let totalFees = BigInt(0);

//   if (operation.transfer && operation.transfer.feeAmount) {
//     totalFees += BigInt(operation.transfer.feeAmount);
//   }

//   if (operation.swap && operation.swap.estimatedAffiliateFee) {
//     const affiliateFee = operation.swap.estimatedAffiliateFee;
//     const [amount] = affiliateFee.split(/(?=\D+$)/);
//     totalFees += BigInt(amount || '0');
//   }

//   // Add gas fees from the execution result
//   if (result.data?.fees?.amount) {
//     for (const fee of result.data.fees.amount) {
//       totalFees += BigInt(fee.amount);
//     }
//   }

//   return { totalFees: totalFees.toString() };
// };

// const findStepHashForOperation = (operationIndex: number): string | null => {
//   const route = get(transactionRouteAtom);
//   for (const step of route.steps) {
//     const stepLog = get(getStepLogAtom(step.hash));
//     if (stepLog?.operationIndex === operationIndex) {
//       return step.hash;
//     }
//   }
//   return null;
