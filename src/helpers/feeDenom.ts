import { SYMPHONY_MAINNET_ASSET_REGISTRY } from '@/constants';
...
export const getValidFeeDenom = (sendDenom?: string, exchangeAssets: Asset[] = []): string => {
  ...
  const isNativeCoin = sendDenom === SYMPHONY_MAINNET_ASSET_REGISTRY.note.denom;
  ...
  console.log(
    `sendDenom is invalid or not a fee token. Defaulting to: ${SYMPHONY_MAINNET_ASSET_REGISTRY.note.denom}`,
  );

  // Default to note if sendDenom isn't valid fee token
  return SYMPHONY_MAINNET_ASSET_REGISTRY.note.denom;
};
